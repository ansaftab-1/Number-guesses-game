...
const play = () => {
  ...
  if (userGuess != answer) {
    if (userGuess < answer) {
      hint.innerHTML = "Too low try Aain! ";
    } else {
    ...
    hint.innerHTML = `comgratulation! <br> The numberwas<span>${answer}</span>.<br>You gueesed the numberin <span> ${noOfGuesses}</span>triess.`;
...
const init = () => {
  ...
  noOfGuesses = 0;
  guessedNumsArr = [];
  noOfGuessesRef.innerHTML = "No o Guesses: 0";
